...
/*
 * Regex to dectect whether there's at least one forward slash after the domain part of the link
 *
 * See for details: https://regex101.com/r/FQRya7/2/
 */
var forwardSlashAfterDomainRegex = /(?:\w+:\/\/)?[a-zA-Z0-9\.-]+\/.*/;
...
function inLinkifyLink (position, fullText, linkify) {
  ...
  // a valid linkify link and there's at least one forward slash between the domain
  // and the mention, the mention is part of the linkify link.
  return linkify.test(prefix) && prefix.match(forwardSlashAfterDomainRegex);
}
