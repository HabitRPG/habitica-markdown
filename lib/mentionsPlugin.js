 ...
 */
function partOfLinkOrEmail (pretext, text, linkify) {
  ...
  var match = linkify.match(possibleLink);
...
function parseMentions (state, silent) {
  ...
  if (!match || partOfLinkOrEmail(state.src.substr(0, state.pos), text, state.md.linkify)) {
    return false;
  }
  var mention = match[0];
